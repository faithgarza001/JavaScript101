...
// Exercise 17
// Write a function definition named isPositiveOdd that takes in a number returns true or false if the value is both greater than zero and odd
...
function hasVowels(input){
  return  input.includes(/[aeiouy]/g); 
}
...
function endsWithVowel(string){
  return ['a', 'e', 'i', 'o', 'u'].lastIndexOf(string[0].toLowerCase()) !== -1;
 }
...
// function startsAndEndwithVowel(){
//   return
// }
...
//Test not working even though getting expected results
...
function thirdToLast(params){
  return (params.slice(-3))[0];
}
...
//The expected results are right, but the test is saying incorrect.
...
sequence.splice( 1, sequence.length - 2 )
  return sequence
}
...
function firstToLast(array){
  ...
  var toLast = array.push(firstElement);
  return array;
}
...
let positiveNumbersCount = arr.filter(v => v > 0);
...
function onlyNegativeNumbers(arr) {
 let negativeNumbersCount = arr.filter(v => v < 0);
...
function countNegatives(inputs) {
  let counter = 0;
  for (const input of inputs) {
...
function countPositives(inputs) {
  let counter = 0;
  for (const input of inputs) {
    if (input > 0) counter += 1;
  }
...
function onlyPositiveEvens(array){
  ...
  for (let i = 0; i < array.length; i++){
    if ((array[i] % 2 === 0 && array[i] > 0) ||( array[i].isPositive)){
      arr.push(array[i]);
    }
  }
  return arr;
...
//  var result = books[0];
// for (var i = 1; i < books.length; i++) {     // iterate from the second on
//     if (books[i].Price > result.Price) { // check price, if a greater price found
//         result = books[i];               // replace result with actual object
//     }
//   return result;
// }
// }
...
// function highestPriceBook(books){
//  var result = books.reduce(function (a, b) {
...
//   if (shoppingCart.hasOwnProperty(item)) count++;
//   }
//   return item;
// }
...
//     var price1 = 0;
//   var i = 0;
//  for (let i = 0; i < shoppingCart.items.price; i++); 
//      price1 += shoppingCart.items[i].price;
// return price1; 
   var priceSum = 0;
    for (var i = 0; i < shoppingCart.items.length; i++) {
         priceSum += shoppingCart.items[i].price;
    ...
    return priceSum/shoppingCart.items.length;
}
...
function getAverageSpentPerItem(shoppingCart){
  ...
  for(var i = 0; i < shoppingCart.length; i++){
    bucket += x.items[i].price
  }
// var shoppingCart = shoppingCart.length;
...
//     if (!this.loadCart || price.value > this.loadCart.value) {
//         this.loadCart = price;
//     }
//   return (loadCart);
// }
